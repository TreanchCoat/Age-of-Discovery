...
## Returns a [PackedByteArray] from the provided data. Godot 4.4+ requires the push
## constant size to match the shader's block size *exactly* (it used to be padded up
## to a 16-byte multiple). All shader push-constant blocks here are tightly packed and
## 4-byte aligned, so we supply exactly len(data)*4 bytes.
