...
func init_gpu(num_cascades : int) -> void:
	...
	# [ocean port] DIAGNOSTIC — prints which compute shader (if any) failed to build.
	# An invalid shader => a null pipeline => the "pipeline is null" spam. Remove later.
	print("[ocean] spectrum_compute valid=",  spectrum_compute_shader.is_valid())
	print("[ocean] fft_butterfly   valid=",  fft_butterfly_shader.is_valid())
	print("[ocean] spectrum_modulate valid=", spectrum_modulate_shader.is_valid())
	print("[ocean] fft_compute     valid=",  fft_compute_shader.is_valid())
	print("[ocean] transpose       valid=",  transpose_shader.is_valid())
	print("[ocean] fft_unpack      valid=",  fft_unpack_shader.is_valid())
